...
class TmpTest {
    ...
    // back_up : (nat) -> (variant { eof : NFTBackupChunk; data : NFTBackupChunk }, ) query;
    @Test
    fun service() {
        val input = """
            type Nft_Canister = service {
                ...
                collection_nft_origyn : (opt vec record { text; opt nat; opt nat }) -> (CollectionResult,) query;
                collection_secure_nft_origyn : (opt vec record { text; opt nat; opt nat },) -> (CollectionResult);
                collection_update_batch_nft_origyn : (vec ManageCollectionCommand) -> (vec OrigynBoolResult,);
                collection_update_nft_origyn : (ManageCollectionCommand) -> (OrigynBoolResult,);
                cycles : () -> (nat) query;
                dip721_balance_of : (principal) -> (nat) query;
                dip721_custodians : () -> (vec principal) query;
                dip721_is_approved_for_all : (principal, principal) -> (DIP721BoolResult,) query;
                dip721_logo : () -> (opt text) query;
                dip721_metadata : () -> (DIP721Metadata) query;
                dip721_name : () -> (opt text) query;
                dip721_operator_token_identifiers : (principal) -> (DIP721TokensListMetadata,) query;
                dip721_operator_token_metadata : (principal) -> (DIP721TokensMetadata) query;
                dip721_owner_of : (nat) -> (OwnerOfResponse) query;
                dip721_owner_token_identifiers : (principal) -> (DIP721TokensListMetadata,) query;
              ...
              governance_batch_nft_origyn : (vec GovernanceRequest) -> (vec GovernanceResult,);
              governance_nft_origyn : (GovernanceRequest) -> (GovernanceResult);
              history_batch_nft_origyn : (vec record { text; opt nat; opt nat }) -> (vec HistoryResult,) query;
              history_batch_secure_nft_origyn : (vec record { text; opt nat; opt nat }) -> (vec HistoryResult,);
              ...
              http_request_streaming_callback : (StreamingCallbackToken) -> (StreamingCallbackResponse,) query;
              ...
              market_transfer_batch_nft_origyn : (vec MarketTransferRequest) -> (vec MarketTransferResult,);
              market_transfer_nft_origyn : (MarketTransferRequest) -> (MarketTransferResult,);
              metadata : () -> (DIP721Metadata) query;
              metadataExt : (EXTTokenIdentifier) -> (EXTMetadataResult) query;
              mint_batch_nft_origyn : (vec record { text; Account }) -> (vec OrigynTextResult,);
              mint_nft_origyn : (text, Account) -> (OrigynTextResult);
              nftStreamingCallback : (StreamingCallbackToken) -> (StreamingCallbackResponse,) query;
              ...
              sale_info_batch_nft_origyn : (vec SaleInfoRequest) -> (vec SaleInfoResult,) query;
              ...
              stage_library_batch_nft_origyn : (vec StageChunkArg) -> (vec StageLibraryResult,);
        ...
        """.trimIndent()
        CandidFileParser.parseFile(input)
    }
}